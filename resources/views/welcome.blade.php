                ...
                @foreach($fotos as $foto)
                    <div class="container p-3" id="container-{{ $foto->id }}">
                            <h5 class="nama-user-h5" style="position: relative;">
                                ...
                                <span class="badge badge-pill" style="position: absolute; top: 0; right: 0; background-color: #4a55a2; color: white;">
...
<script>
    ...
    @auth
        ...
        likeButtons.forEach(function (button) {
            button.addEventListener('click', function (event) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        // Ubah ikon menjadi solid merah jika tombol disukai
                        if (!isLiked) {
                            button.innerHTML = '<i class="fas fa-heart"></i> Like';
                            button.style.color = 'red';
                            localStorage.setItem('liked_' + fotoId, true); // Simpan status liked di localStorage
                        } else {
                            button.innerHTML = '<i class="far fa-heart"></i> Like';
                            button.style.color = '';
                            localStorage.removeItem('liked_' + fotoId); // Hapus status liked dari localStorage
                        }
            ...
            // Periksa apakah tombol "like" sudah disukai sebelumnya dan atur ulang warna ikon sesuai statusnya
            var fotoId = button.getAttribute('data-foto-id');
            if (localStorage.getItem('liked_' + fotoId)) {
                button.innerHTML = '<i class="fas fa-heart"></i> Like';
                button.style.color = 'red';
            }
        });
